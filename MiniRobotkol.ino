...
//arraystep = dizide hangi pozisyonda olduğumuzu saklıyor
//arrayMax = dizide maks kaç adım olacağı
//countverz = hareketler arasındaki süre
//Taster = Butonun durumu
//stepsMax = servonun maks hareket mesafesi
//steps = hafızaya alınan durumlar arasındaki her bir hareket
unsigned int  verz = 0;
//millis hesapları için değişkenler tanımlanıyor
...
int joint2[180];
int joint3[180];
int top = 179; // dizinin sonuna yazmayı engellemek için konulmuş
// durum nedir oynatma mı adım mı? 
...
void loop() 
{
  ...
  else if(playmode) // oynatma modundaysak
  {
    if (Step) // bir sonraki pozisyonları diziden oku
    {
      digitalWrite(13, HIGH); //LED'i yak
      if (arrayStep < arrayMax) // veriye sondan değil dizinin başından erişmeliyiz.
      {
        arrayStep += 1; // bir sonraki dizi pozisyonu
        Read(); // diziden verileri oku
        calculate(); // servolar içindeki en çok hareket edecek servoyu bul ve diğer 3ünün hareketini ona uydur ki hepsi aynı anda hareketi tamamlasın
        Step = 0;
        digitalWrite(13, LOW);  //ledi söndür
      }
      else // dizi bittiyse baştan başla hareketi döngüsel yapıyoruz
      {
        arrayStep = 0; // sıfırladık
        calc_pause(); // hareketler arası beklemeler
        countverz = 0; // gecikme için kullanılan değişken
        while(countverz < verz) // verz ise calc_pause(); fonsiyonundan gelen değer
        { // bir sonraki tura kadar bu döngüde bekletiyoruz
          countverz += 1;
          calc_pause();
...
// ---------------------------------------------------------------------------------------- alt fonksiyonlar
void calc_pause() // hareketler yapıldıktan sonra ne kadar bekleme yapılacağını hesaplıyor
{
    readPot();
    temp = SensVal[3];
    if (temp < 0) temp = 0;
    temp = map(temp, 0, 680, 0 ,5); 
    verz = Delay[temp]; // verz = saniye bazında bekleme
    Serial.print(temp);
          Serial.print(" ");
          Serial.print(verz);
          Serial.print(" ");
...
void readPot() // pot değerleri okunuyor ve bazı mekanik düzeltme offsetleri ekleniyor
{
...
void mapping() // servolara potanslardan gelen değer yerine mili saniye değerlerinin atanması lazım
{
...
void record()
{
    joint0[arrayStep] = ist[0]; // servoların pozisyonları diziye kayıt ediliyor
...
void Read()
{
    sol[0] = joint0[arrayStep]; // değerleri diziden okuyoruz
...
void move_servo()
{       
  servo_0.writeMicroseconds(ist[3]); // servolara microsaniye değerleri gönderiliyor
...
void calculate()
{
      // her bir servo için hareket mesafesi
      ...
      // en uzun hareket eden iki bekleme ve yeni hamle süresinden daha kısa olmalı
      ...
      if (stepsMax < 500) // eğer en uzun hareket <500 ise delay 1200 diğer durumlarda delay 600ms
       ...
       // herbir servo için micro step değeri hesaplanıyor
       // her bir servo hareketi kendi değişik değerleri ile en uzun hamle zamanı içinde gerçekleşmeli (stepsMax times).
       // Bu sayede tüm servolar hareketin bitişine aynı anda ulaşacak ve smooth bir hareket olacak
...
{
    steps += 1;
    if (steps < stepsMax) // diziye sondan ulaşmamak için
    {
      //time = del*5;
      if(steps == 20) time = del*4;         // ramp up 
      else if(steps == 40) time = del*3;    // zamanı ana loop içnde micro stepler yapılana kadar beklenen süre
      else if(steps == 80) time = del*2;    
      else if(steps == 100) time = del-1;    // kasıtlı olarak del-1 çarpı değil
      
      if(steps == stepsMax-200) time = del*2;        // ramp down durduruluyor
      else if(steps == stepsMax-80) time = del*3;
      else if(steps == stepsMax-40) time = del*4;
      else if(steps == stepsMax-20) time = del*5;
      
      ist[0] += dir[0]; // yeni pozisyonlar atanıyor
      ist[1] += dir[1];
      ist[2] += dir[2];
      ist[3] += dir[3];
...
void data_out() // kaydedilen verileri serial monitöre yazmak için
{
  int i = 0;
  while(i < arrayMax)
  {
...
void Button() // Butonu kontrol ediyoruz tek mi çift mi tıklanıyor
{
  if (digitalRead(6) == false)
  {
    delay(1);
    if (digitalRead(6) == true) //basıldıysa
    {
      if (Taster == 0)
      {
        Taster = 1; 
      ...
      else if ((Taster == 1) && (currentMillis - previousMillis3 < 250))//çift mi tek mi anlamak için millis ile zamana bakılıyor
      {
        Taster = 2;
    ...
    if ((Taster == 1) && (currentMillis - previousMillis3 > 1000)) // tek tıklama olduğunu anlıyor ve diziye kayıt yapıyor
    {
      arrayStep += 1;
      arrayMax = arrayStep;
      record();
    ...
    if (currentMillis - previousMillis3 > 2000) // butonun kontrol zamanı resetleniyor
    {
      Taster = 0;
